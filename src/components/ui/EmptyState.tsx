...
/**
 * The sizes the three glyphs below are drawn at. Tailwind needs the whole
 * class name in the source, so the map is written out rather than built from
 * the number: `h-${size}` would compile to nothing.
 *
 * 16 is the empty-state size. The smaller ones stand in for a missing
 * thumbnail — 6 in a grid tile, 8 in a list row, 10 in the upload drop zone —
 * and 24 fills the image panel on a film or camera page.
 */
const ICON_SIZE = {
  6: 'h-6 w-6',
  8: 'h-8 w-8',

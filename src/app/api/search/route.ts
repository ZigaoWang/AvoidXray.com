...
export async function GET(req: NextRequest) {
  ...
  const [cameraRecords, cameraPhotoCounts] = await Promise.all([
    ...
    // Counted separately, off the matched ids, so the count query runs beside
    // the hydration instead of costing a round trip of its own.
    photoCountsByCamera(cameraMatches.map(m => m.id), photoScope),
  ])
  ...
  const cameras = cameraRecords
    ...
    .map(c => ({
      ...
      _count: { photos: cameraPhotoCounts.get(c.id) ?? 0 },
  ...
  const [filmRecords, filmPhotoCounts] = await Promise.all([
    ...
    photoCountsByFilmStock(filmMatches.map((m) => m.id), photoScope),
  ])
  ...
  const films = filmRecords
    ...
    .map((f) => ({
      ...f,
      _count: { photos: filmPhotoCounts.get(f.id) ?? 0 },
      matchedAlias: filmMatches.find((m) => m.id === f.id)?.matchedAlias ?? null,
    }))
